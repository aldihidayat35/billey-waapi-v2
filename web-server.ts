...
// ============================================
// OTP Login Storage (in-memory, resets on server restart)
// ============================================
const otpStorage: Map<string, { otp: string, expiresAt: number, targetPhone: string }> = new Map()

function generateOTP(): string {
	return Math.floor(1000000000 + Math.random() * 9000000000).toString() // 10 digit OTP
}
...
// API: Get connected sessions for OTP login (public endpoint)
app.get('/api/auth/connected-sessions', (req, res) => {
	try {
		...
		const connectedSessions = allSessions
			...
			.map(s => ({
				id: s.id,
				phoneNumber: s.phoneNumber || s.user?.id?.replace(/:.+/, '') || '-',
				name: s.user?.name || s.id
			}))
		
		res.json({
			success: true,
			hasConnectedSessions: connectedSessions.length > 0,
...
app.post('/api/auth/send-otp', async (req, res) => {
	try {
		const { sessionId, targetPhone } = req.body

		if (!sessionId || !targetPhone) {
			return res.status(400).json({ 
				success: false, 
				error: 'Session dan nomor tujuan wajib diisi' 
			})
		}

		// Validate session is connected
		const sessions = sessionManager.getAllSessions()
		const session = sessions.find(s => s.id === sessionId && s.isConnected)
		
		if (!session) {
			return res.status(400).json({ 
				success: false, 
				error: 'Session tidak terhubung' 
			})
		}
		...
		// Store OTP
		otpStorage.set(formattedPhone, { otp, expiresAt, targetPhone: formattedPhone })

		// Send OTP via WhatsApp
		const jid = `${formattedPhone}@s.whatsapp.net`
		const message = `🔐 *Kode OTP Login WhatsApp Gateway*\n\nKode OTP Anda: *${otp}*\n\n⏰ Berlaku selama 5 menit\n⚠️ Jangan bagikan kode ini kepada siapapun!`

		try {
			await sessionManager.sendMessage(sessionId, jid, message)
			
			console.log(`📤 OTP sent to ${formattedPhone} via session ${sessionId}`)
			
			res.json({
				success: true,
				message: `OTP telah dikirim ke ${targetPhone}`,
			...
			return res.status(500).json({ 
				success: false, 
				error: 'Gagal mengirim OTP. Pastikan nomor WhatsApp valid.' 
			})
		}
	} catch (error: any) {
...
app.post('/api/auth/verify-otp', (req, res) => {
	try {
		...
		// OTP valid - delete it and login as admin
		otpStorage.delete(formattedPhone)

		// Get admin user (first adminwa user)
		const adminUser = userDb.getAll({ role: 'adminwa', limit: 1 }).users[0]
		
		if (!adminUser) {
			return res.status(500).json({ 
				success: false, 
				error: 'Tidak ada admin user ditemukan' 
			})
		}
		...
		console.log(`✅ OTP Login successful for phone ${formattedPhone}`)

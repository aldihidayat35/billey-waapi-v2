...
function showCleanupModal() {
    if (!cleanupModal) {
        cleanupModal = new bootstrap.Modal(document.getElementById('cleanupModal'));
    }
    
    // Reset result area
    document.getElementById('cleanup-result').classList.add('d-none');
...
async function loadCleanupStats() {
    try {
        const response = await fetch('/api/database/stats', { credentials: 'include' });
        const data = await response.json();
        ...
        if (data.success) {
            ...
            // Update table stats
            const container = document.getElementById('cleanup-table-stats');
            let html = '';
        ...
        document.getElementById('cleanup-current-size').textContent = 'Error loading';
    }
}

function getTableIcon(tableName) {
    const icons = {
        'message_logs': 'bi-chat-dots',
        'session_logs': 'bi-phone',
        'chat_templates': 'bi-file-text',
        'group_exports': 'bi-people',
        'auto_reply_rules': 'bi-robot',
        'auto_reply_logs': 'bi-clock-history',
        'auto_reply_cooldowns': 'bi-hourglass'
    };
...
async function runCleanup() {
    ...
    try {
        ...
        if (data.success) {
            ...
            document.getElementById('cleanup-result-details').innerHTML = `
                <ul class="mb-0">
                    <li>Message logs dihapus: <strong>${data.result.messagesDeleted}</strong></li>
                    <li>Session logs dihapus: <strong>${data.result.sessionLogsDeleted}</strong></li>
                    <li>Auto reply logs dihapus: <strong>${data.result.autoReplyLogsDeleted}</strong></li>
                    <li>Cooldowns dihapus: <strong>${data.result.cooldownsDeleted}</strong></li>
                    ${data.result.mediaCleared ? `<li>Media data dihapus: <strong>${data.result.mediaCleared}</strong></li>` : ''}
                    <li class="mt-2">
                        <strong>Ukuran sebelum:</strong> ${data.result.sizeBefore.mb} MB<br>
                        <strong>Ukuran sesudah:</strong> ${data.result.sizeAfter.mb} MB<br>
                        <strong class="text-success">Ruang dihemat: ${data.result.spaceSaved.mb} MB</strong>
                    </li>
                </ul>
            `;
            ...
            showToast('success', `Cleanup berhasil! Ruang dihemat: ${data.result.spaceSaved.mb} MB`);
        } else {
            Swal.fire({
                icon: 'error',
                title: 'Error',
        ...
        btn.disabled = false;
        btn.innerHTML = '<i class="bi bi-trash3 me-1"></i>Jalankan Cleanup';
    }
}
...
async function runVacuum() {
    ...
    btn.disabled = true;
    btn.innerHTML = '<span class="spinner-border spinner-border-sm me-2"></span>Vacuuming...';
    ...
    try {
        ...
        if (data.success) {
            Swal.fire({
                icon: 'success',
                title: 'Vacuum Selesai!',
                html: `
                    <p>Database telah dioptimasi.</p>
                    <ul class="text-start">
                        <li>Ukuran sebelum: <strong>${data.sizeBefore.mb} MB</strong></li>
                        <li>Ukuran sesudah: <strong>${data.sizeAfter.mb} MB</strong></li>
                        <li class="text-success">Ruang dihemat: <strong>${data.spaceSaved.mb} MB</strong></li>
                    </ul>
                `,
                timer: 5000
            });
        ...
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Vacuum error: ' + error.message
        });
    } finally {
        btn.disabled = false;
        btn.innerHTML = '<i class="bi bi-gear me-1"></i>Vacuum Only';
    }
}
...
export class SessionManager {
	...
	/**
	 * Auto-reconnect all sessions that have saved auth credentials
	 * This should be called after server starts
	 */
	async autoReconnectAllSessions(): Promise<void> {
		...
		try {
			...
			for (const folder of authFolders) {
				...
				try {
					...
					// Start session with QR type (will auto-connect if creds are valid)
					await this.startSession(sessionId, 'qr')
					
					// Wait 2 seconds between sessions to prevent rate limiting
					await new Promise(resolve => setTimeout(resolve, 2000))
					
				} catch (error: any) {
					console.error(`❌ Failed to auto-reconnect session ${sessionId}:`, error.message)
				}
			}
			...
			console.error('❌ Error in autoReconnectAllSessions:', error)
		}
	}
